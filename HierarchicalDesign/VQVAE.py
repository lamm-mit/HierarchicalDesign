#########################################################
# Define codebook model and VQ-VAE with attention
#########################################################
...
class VQVAEModel(nn.Module):
    ...
    def forward(self, x):
        ...
        return x_hat, indices, commitment_loss#, codebook_loss, perplexity 
    ...
    def decode (self, z): #itake predicted z and snap it to the quantized ones, then calculate x_hat
        x_hat_nonquant = self.decoder(z) #non-snapped
        return x_hat_nonquant
    
    def decode_snapped (self, z): #take predicted z and snap it to the quantized ones, then calculate x_hat
        
        z_quantized, indices, commitment_loss,    = self.codebook(z)
        
        x_hat = self.decoder(z_quantized)
        
        return x_hat, z_quantized 
        
#########################################################
# CODE BASE: CNN/Attention layers
#########################################################
...
class Decoder_Attn(nn.Module):
    def __init__(self,  image_channels=3, latent_dim=128,channels = [512, 256, 256, 128, 128] ):
        ...
        layers.append(GroupNorm(channels=in_channels ))
        layers.append(Swish())
